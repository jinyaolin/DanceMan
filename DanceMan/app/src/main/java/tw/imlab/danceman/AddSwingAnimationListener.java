...
import tw.imlab.danceman.animation.SwingAnimation;
import tw.imlab.danceman.tweener.InQuadTweener;
import tw.imlab.danceman.tweener.Tweener;

public class AddSwingAnimationListener implements OSCListener {
    AnimationView animationView;

    public AddSwingAnimationListener (AnimationView view) {
        animationView = view;
    }
    ...
    public void acceptMessage (Date time, OSCMessage message) {
        ...
        for (int i = 0; i < indexString.length(); ++i) {
            if (indexString.charAt(indexString.length() - 1 - i) == '1') {
                Animation animation = new SwingAnimation(animationView.getDancer(i),tweener, elapsedTime * 1000000, angle);
